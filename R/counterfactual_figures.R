...
tikz(file = "~/Dropbox/Research Projects/WelfareMetaAnalysis/Figures/DecompCounterfactual.tex",width=5,height = 3.5)
print(g)
dev.off()

tikz(file = "~/Dropbox/Research Projects/WelfareMetaAnalysis/Figures/DecompCounterfactualPaper.tex",width=6,height = 4)
print(g+theme_minimal() + theme(legend.position = "bottom",legend.title = element_blank()))
dev.off()



d <- read.csv("~/Dropbox/Research Projects/WelfareMetaAnalysis/julia/output/decomp_counterfactual2.csv")
...
tikz(file = "~/Dropbox/Research Projects/WelfareMetaAnalysis/Figures/DecompCounterfactual2.tex",width=5,height = 3.5)
print(g)
dev.off()

# -- now depict the non_selected counterfactual --- #s
d <- read.csv("~/Dropbox/Research Projects/WelfareMetaAnalysis/julia/output/non_selected_counterfactual.csv") %>%
  select(-source) %>%
  rename(source = case) %>%
  mutate(case = "Non-Selected")

d0 <- read.csv("~/Dropbox/Research Projects/WelfareMetaAnalysis/julia/output/decomp_counterfactual.csv") %>%
...
tikz(file = "~/Dropbox/Research Projects/WelfareMetaAnalysis/Figures/non_selected_counterfactual.tex",width=5,height = 4)
