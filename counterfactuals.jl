...
function counterfactual_chunk(p,EM::Vector{EM_data},MD,data::Vector{likelihood_data},n_idx)
    ...
    for md in MD
        ...
        for n in n_idx[md.case_idx]
            ...
            initialize_exante!(π0,EM[n],s_inv) #<- get initial dist from posterior
...
function counterfactual_stats(p,em::EM_data,md::model_data,data::likelihood_data)
    ...
    d = [d;DataFrame(DataFrame(variable = "AFDC",value = A,year = year,Q = Q))]
    d = [d;DataFrame(DataFrame(variable = "Earn",value = E,year = year,Q = Q))]
