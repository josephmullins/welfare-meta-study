...
function log_likelihood_bench!(x,solvetime::Vector{Float64},M::Matrix{ddc_model},EM::Vector{EM_data},MD::Vector{model_data},p::pars,data::Vector{likelihood_data},n_idx::Vector{Vector{Int64}})
    fill!(LL,0.)
    update!(x,p)
    update!(p,M,∂M,(1,8,9))
...
log_likelihood_bench!(x0,solvetime,M,EM,MD,p,data,n_idx)
