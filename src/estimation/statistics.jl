...
function get_choice_state_distribution!(Π::SparseMatrixCSC{Float64,Int64},logP,model,p,R::Int64,loc::Int64) #<- need choice probs? T?
    fill!(Π,0.)
    J = 9
...
function exante_model_fit_chunk(p,EM::Vector{EM_data},MD,data::Vector{likelihood_data},n_idx)
    ...
    for md in MD
        ...
        for n in n_idx[md.case_idx]
            ...
            get_choice_state_distribution!(EM[n].q_s,logP,(;K,π0,s_inv,k_inv,Kω,k_idx),p,md.R,loc) #<- nice.
