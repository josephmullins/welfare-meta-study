...
function basic_model_fit_chunk(p::pars,EM::Vector{EM_data},MD,data::Vector{likelihood_data},n_idx)
    ...
    for md in MD
        ...
        for n in n_idx[md.case_idx]
            initialize!(logπτ,EM[n],M[m_idx],p,md,data[n])
            get_choice_state_distribution!(EM[n].q_s,M[m_idx].logP,M[m_idx]) #<- nice.
...
function initialize!(logπτ,EM::EM_data,model::ddc_model,p::pars,md::model_data,data::likelihood_data)
    J = model.J
    fill!(model.π0,0.)
    ...
    for s in nzrange(EM.α,1)
        ...
        if model.π0[k]==0
            model.π0[k] = initial_prob(kA,kη,kτ,logπτ,p,md)
        end
    end
    model.π0 ./= sum(model.π0)
end
