        ...
        for k in 1:K
            ...
            for j in 1:J
                if m.choice_set[j,k,t]
                    ...
                    m.v[j] = utility(S,A,H,F,p,md,kA,kη,kτ,t,eligible)
...
function solve!(m::ddc_model,∂m::ddc_derivative,md::model_data,p::pars,t::Int16,tnow::Int16,tnext::Int16)
    ...
    for k in 1:K
        ...
        @views get_derivatives_du!(m.logP[:,k,t],p.σ,∂m.logP[∂m.u_idx,:,k],∂m.v[∂m.u_idx,:],m.choice_set[:,k,t],m.G)
        # F enters only through v
        @views get_derivatives_du!(m.logP[:,k,t],p.σ,∂m.logP[∂m.F_idx,:,k],∂m.v[∂m.F_idx,:],m.choice_set[:,k,t],m.G)
        # β
        @views get_derivatives_du!(m.logP[:,k,t],p.σ,∂m.logP[∂m.β_idx,:,k],∂m.v[∂m.β_idx,:],m.choice_set[:,k,t],m.G)
        # σ is enters into V both through v and through Emax
        @views get_derivatives_dσ!(m.logP[:,k,t],m.v,p.σ,∂m.logP[∂m.σ_idx,:,k],∂m.v[∂m.σ_idx,:],m.choice_set[:,k,t],m.G,false)
